        ...
        for checkpoint in tqdm(list_checkpoints) : 
            if checkpoint.split('-')[-1].replace('.csv', '')!='0':  
                ...
                for data, d_type in zip(list_data, list_type): 
                    ...
                    if output_translated==True: 
                        df_path = f"{path_translation}/{checkpoint_name}_{d_type}_{args.dist_type}_with_translation_and_refusal_without_rephrase.csv"
                        print(df_path)
                    else : 
                        df_path = f"{path_dist}/{checkpoint_name}_{d_type}_{args.dist_type}.csv"
                    if Path(df_path).is_file(): 
                        if output_translated==True: 
                            print("Read translated data with refusal")
                            data_with_refusal = pd.read_csv(df_path)
                            ...
                            print("Read data")
                            data_with_refusal = pd.DataFrame(pd.read_csv(df_path,  encoding="utf8"))
                            data_with_refusal = data_with_refusal[(data_with_refusal['data_type']!='privacy_fictional')]
                    else : 
                        if output_translated==True: 
                            print("Compute translated refusal")
                            df_path_translation = f"{path_translation}/{checkpoint_name}_{d_type}_{args.dist_type}_with_translation.csv"
                            #if not Path(df_path).is_file(): 
                            #    data_with_translation = translated_low_resource_output(df, tokenizer_translation, model_translation, df_path_translation)
                            #else : 
                            data_with_translation = pd.DataFrame(pd.read_csv(f"{path_translation}/{checkpoint_name}_{d_type}_{args.dist_type}_with_translation.csv",  encoding="utf8"))
                            data_with_translation = data_with_translation[(data_with_translation['data_type']!='privacy_fictional')]
                            data_with_refusal = detect_lg_and_refusal(data_with_translation, checkpoint_name, lg,detector_model,  pipe_detection_lg, df_path, output_translated)
                            data_with_refusal = remove_rephrase_sentence(data_with_refusal,f"{path_translation}/{checkpoint_name}_{d_type}_{args.dist_type}_with_translation_and_refusal_without_rephrase.csv")
                        else : 
                            print("Clean and compute refusals")
                            data = data[(data['data_type']!='privacy_fictional')]
                            clean_data = clean_output_cut_assistant(data)
                            if "detector" in args.dist_type: 
                                data_with_refusal = detect_lg_and_refusal(clean_data, checkpoint_name, lg, detector_model,  pipe_detection_lg, df_path)
                            else : 

...
class ModelBase(ABC):
    def __init__(self, model_name_or_path: str, checkpoint: str):
    ...
    @abstractmethod
    def _load_model(self, model_name_or_path: str, checkpoint:str) -> AutoModelForCausalLM:
        pass
    ...
    def generate_completions(self, dataset, fwd_pre_hooks=[], fwd_hooks=[], batch_size=256, max_new_tokens=64, temperature=0, lg='base'):
        #generation_config = GenerationConfig(max_new_tokens=max_new_tokens, do_sample=temperature > 0, temperature=temperature)
        generation_config = GenerationConfig(max_new_tokens=50,temperature = 0.7, top_p = 0.8, top_k = 20)
        ...
        max_batch_size = int(get_max_batch_size() / 6)
